...
# Clean only known installer outputs and stale temp dirs.
try {
    $finalInstallerLocal = Join-Path $OutDirLocal $OutFileName
    if (Test-Path -LiteralPath $finalInstallerLocal) {
        Remove-Item -LiteralPath $finalInstallerLocal -Force -ErrorAction SilentlyContinue
    }

    Get-ChildItem -LiteralPath $OutDirLocal -Filter 'tmp_inno_out_*' -Directory -ErrorAction SilentlyContinue | ForEach-Object {
...
try {
    ...
    # Always cleanup temp output directory (avoid bin/ litter), even on failures.
    try {
        if (Test-Path -LiteralPath $tmpOutDirLocal) {
            Remove-Item -LiteralPath $tmpOutDirLocal -Recurse -Force -ErrorAction SilentlyContinue
        }
